...
class DoomNetwork(DQNNetworkDef):
    ...
    def create(self, inputs, input_dim, output_dim, scope="CartPoleNetwork", reuse=False):
        ...
        return slim.fully_connected(inputs=flattened, num_outputs=output_dim, scope=scope + "_o", reuse=reuse,
                                    activation_fn=None)
...
def main():
    ...
    # dqn.load(".saves/doom_9.ckpt")
    memory = MemoryReplay(height * width, num_actions, max_saved=10000)
    ...
    for epoch in range(1000):
        ...
        for i in range(10000):
            ...
            s_prime, r, t, _ = env.step(action.reshape([-1]))
            total_reward += r
        ...
        for i in range(min((epoch + 1) * 10, 250)):
